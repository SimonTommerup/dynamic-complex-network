...
class SmallNet(nn.Module):
    def __init__(self, pars_mode, n_points, init_beta, riemann_samples, node_pair_samples, non_intensity_weight=1):
        super().__init__()
        self.beta = nn.Parameter(self.init_beta(torch.zeros(size=(n_points,1))), requires_grad=True)
    ...
    def lambda_fun(self, t, u, v):
        ...
        return torch.exp(self.beta[u] + self.beta[v] - d)
    ...
    def forward(self, data, t0, tn):
        ...
        #\beta_i+\beta_j-||z_i(t)-z_j(t)||

        for u, v, event_time in data:
            u, v = to_long(u, v) # cast to int for indexing
            event_intensity += self.beta[u] + self.beta[v] - self.get_dist(event_time, u, v)
