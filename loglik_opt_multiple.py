...
class SmallNet(nn.Module):
    def __init__(self, n_points, tn_train, tn_test):
        ...
        self.z0 = nn.Parameter(torch.rand(size=(n_points,2))*5)
    ...
    def eval_integral(self, i, j, z, v, alpha, beta):
        if self.eval():
            t0 = self.tn_train
            tn = self.tn_test
        elif self.train():
    ...
    def forward(self, data, weight=1):
        eps = 1e-7
        event_intensity = 0.
        ...
        for u, v in zip(ind[0], ind[1]):
            non_event_intensity += self.eval_integral(u, v, self.z0, self.v0, alpha=self.alpha, beta=self.beta)
...
for epoch in range(num_epochs):
    ...
    print(f"Epoch {epoch+1} train loss: {loss.item()/ len(training_data)} test loss: {tloss.item() / len(test_data)}")
    print(f"time: ", time.time()-t0)
    train_loss.append(loss.item() / len(training_data))
...
# %%
torch.save(net.state_dict(), "model-50-epoch-perfect-alpha-beta-init.pth")
# %%
...
print(len(data_set))
# %%
PATH = "model-50-epoch-perfect-alpha-beta-init.pth"
n_points = 4
tn_train = 0.
tn_test = 15.
...
ax[0].plot(traint, restrain, color="red")
ax[0].plot(traint, gttrain, color="blue")
ax[0].set_title("Train")
ax[1].plot(testt, restest, color="red")
ax[1].plot(testt,gttest, color="blue")
ax[1].set_title("Test")
plt.show()
# %%
